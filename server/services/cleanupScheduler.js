...
class CleanupScheduler {
  ...
  start() {
    ...
    console.log(
      "   - Daily cleanup: 3:00 AM (notifications, old activity logs)",
    );
  ...
  async runWeeklyCleanup() {
    ...
    try {
      ...
      const totalReduction =
        statsBefore.activityLogs +
        statsBefore.notifications +
        statsBefore.comments +
        statsBefore.completedTasks -
        (statsAfter.activityLogs +
          statsAfter.notifications +
          statsAfter.comments +
          statsAfter.completedTasks);
  ...
  async cleanupCommentsThoroughly() {
    try {
      ...
      const oldArchivedCases = await Case.findAll({
        where: {
          [Op.or]: [{ isArchived: true }, { phase: "closed" }],
          updatedAt: {
            [Op.lt]: cutoffDate,
          },
        },
